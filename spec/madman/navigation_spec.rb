...
describe Navigation do
  ...
  describe '#toc' do
    it "returns an array representation of the table of contents" do
      expect(subject.toc.to_yaml).to match_fixture('navigation/toc')
    end
    ...
    context "when depth is limited" do
      ...
      it "returns only items up to the limit" do
        expect(subject.toc.to_yaml).to match_fixture('navigation/toc-depth-1') 
      end      
    end
  end
  ...
  describe '#markdown' do
    it "returns a markdown table of contents" do
      expect(subject.markdown).to match_fixture('navigation/markdown')
    end
  end
end